...
public class Survivor extends PieceDynamic {
	...
	public void move() {
		if(Math.abs(positions[0][0]-positions[1][0])<=vision && (Math.abs(positions[1][0]-positions[1][1])<=vision)){// se esta proximo do murderer
			...
			else if(positions[0][0]<=positions[1][0] && positions[1][0]<11) positions[1][0]++;
			if(positions[0][1]>=positions[1][1] && positions[1][1]>0) positions[1][1]--;
			else if(positions[0][1]<=positions[1][1] && positions[1][1]<11) positions[1][1]++;
			return;
		}
